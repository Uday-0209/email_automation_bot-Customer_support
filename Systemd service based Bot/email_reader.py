...
#     """Fetch and print email sender + subject."""
...
#     if os.path.exists('/home/smddc/Documents/new_pro/email_classifier/token.json'):
...
#         else:
#             flow = InstalledAppFlow.from_client_secrets_file('/home/smddc/Documents/new_pro/email_classifier/credentials.json', SCOPES)
#             creds = flow.run_local_server(port = 0)
#         with open('/home/smddc/Documents/new_pro/email_classifier/token.json', 'w') as token:
#             token.write(creds.to_json())

#     service = build('gmail', 'v1', credentials = creds)

#     results = service.users().messages().list(userId = 'me', q = 'is:unread', maxResults = 30).execute()
#     seen_ids = []

#     messages = results.get('messages', [])
#     for m in messages:
#         if m['id'] not in seen_ids:
#             process_new_message(service, m['id'])
#             seen_ids.add(m['id'])
...
#     print('Your recent emails')

#     for msg in messages:
#         print(msg)
#         msg_data = service.users().messages().list(userId = 'me', id = msg['id']).execute() 
...
from googleapiclient.discovery import build
import re
from reply_generator import run_generator
...
#     raw_data = get_email_body(msg)
#     body = get_clean_text(raw_data)
...
def authenticate():
    creds_read = None
    creds_send = None
    if os.path.exists('/home/smddc/Documents/new_pro/email_classifier/token_read.json'):
        creds_read = Credentials.from_authorized_user_file('/home/smddc/Documents/new_pro/email_classifier/token_read.json', SCOPES_read)
    ...
    if not creds_read or not creds_read.valid:
        if creds_read and creds_read.expired and creds_read.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                "/home/smddc/Documents/new_pro/email_classifier/credentials.json", SCOPES_read
            )
            creds_read = flow.run_local_server(port=0)

        with open("/home/smddc/Documents/new_pro/email_classifier/token_read.json", "w") as f:
            f.write(creds_read.to_json())
            
            
    if os.path.exists('/home/smddc/Documents/new_pro/email_classifier/token_send.json'):
        creds_send = Credentials.from_authorized_user_file('/home/smddc/Documents/new_pro/email_classifier/token_send.json', SCOPES_send)
    ...
    if not creds_send or not creds_send.valid:
        if creds_send and creds_send.expired and creds_send.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                "/home/smddc/Documents/new_pro/email_classifier/credentials.json", SCOPES_send
            )
            creds_send = flow.run_local_server(port=1)

        with open("/home/smddc/Documents/new_pro/email_classifier/token_send.json", "w") as f:
        ...
        for m in messages:
            ...
            if msg_id not in seen_ids:
                ...
                if output is not None:
                    ...
                    print('\nThe reply email is generating via llama3.2 8b model')
                    llm_generation = run_generator.generate_email(int(k))
                    mail = ''
                    ...
                    print('\nSending Email.....')
                    send_email(service_send, Sender, 'Replay for error', mail)
